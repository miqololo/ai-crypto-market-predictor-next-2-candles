/**
 * Feature metadata: category and short description for LLM1 features.
 * Synced with backend canonical features. Unknown features default to "Other".
 */
export const FEATURE_META: Record<string, { category: string; description: string }> = {
  ...
  strategy_multi_confirmation: {
    category: 'Strategy',
    description: 'Ensemble score [0,1] — тренд + моментум + волатильность',
  },
  strategy_divergence_reversal: {
    category: 'Strategy',
    description: 'Divergence reversal score [0,1] — RSI/MACD vs price divergence',
  },
  strategy_sentiment_boosted_trend: {
    category: 'Strategy',
    description: 'Trend + sentiment boost [0,1]',
  },
  strategy_risk_adjusted_momentum: {
    category: 'Strategy',
    description: 'Risk-adjusted momentum score [0,1] — моментум × ATR',
  },
  strategy_supertrend_strength: {
    category: 'Strategy',
    description: 'Supertrend + strength score [0,1]',
  },
}

const DEFAULT_META = { category: 'Other', description: 'Custom feature' }

export function getFeatureMeta(name: string) {
  return FEATURE_META[name] || DEFAULT_META
}

/** localStorage key for feature enabled state: Record<featureName, boolean> */
export const STORAGE_KEY = 'llm1_features_enabled'

/** localStorage key for feature weights: Record<featureName, weightString> */
export const PRIORITIES_STORAGE_KEY = 'llm1_feature_priorities'
